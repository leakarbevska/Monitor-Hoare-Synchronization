/*
 * Producteur-consommateur, un producteur depose deux messages consecutifs
 * VERSION 1 -Marche si on specifie deux threads pour chaque Producteaur dans l'execution (nbFoisProd=2)  !IMPORTANT!
 *
...
/*--Q1--*/
int nbFoisProd, nbFoisConso; //fois prod/cons par thread
/*--Q2--*/
int nbLibres;   //nb cases libres dans buffer
pthread_mutex_t mutex = PTHREAD_MUTEX_INITIALIZER; //mutex pour func de moniteur
pthread_cond_t cond_CasesLibre = PTHREAD_COND_INITIALIZER;
pthread_cond_t cond_CasesPlein = PTHREAD_COND_INITIALIZER;

/*--Q5--*/
int nbCasesLibres;
int nbDepotPerProd  = 0;    //nombre des depots effectues par le meme prod
int depotEnCours    = false;     // s'il y a un producteur active
int rangProdEnCours = 0; // rang du producteur active
...
void deposer (TypeMessage leMessage, int rangProd) {
  ...
  while(nbLibres == 0 || (depotEnCours && rangProdEnCours != leMessage.rangProd))
    pthread_cond_wait (&cond_CasesLibre, &mutex);
  depot(&leMessage);
  nbLibres--;
  nbDepotPerProd++;
  rangProdEnCours = leMessage.rangProd;
  depotEnCours = true;
  ...
  if(nbDepotPerProd == 2){
    depotEnCours = false;
    nbDepotPerProd = 0;
    pthread_cond_signal(&cond_CasesPlein);
  }
...
void retirer (TypeMessage *unMessage, int rangConso) {
  pthread_mutex_lock(&mutex);
  while(nbLibres == nbCases || depotEnCours)
    pthread_cond_wait(&cond_CasesPlein, &mutex);
  retrait(unMessage);
  nbLibres++;
  printf("\t\tConso %d : Message a ete lu = [T%d] %s (de %d)\n",
         rangConso, unMessage->type, unMessage->info, unMessage->rangProd);
  pthread_cond_signal(&cond_CasesPlein);
  pthread_cond_signal(&cond_CasesLibre);
  pthread_mutex_unlock(&mutex);
}
