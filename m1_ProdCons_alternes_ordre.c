/*
 * Producteur-consommateur, consommateurs demandent à retirer un message d’un certain type et où leurs retraits doivent être effectués dans l’ordre des dépôts
 *
...
int indexLastMessage = -1;

pthread_cond_t caseLibre;
pthread_cond_t consommateurTrue  = PTHREAD_COND_INITIALIZER;
pthread_cond_t consommateurFalse = PTHREAD_COND_INITIALIZER;
pthread_cond_t casePleinPour[2];
...
void deposer (TypeMessage leMessage, int rangProd) {
  ...
  if(nbLibres == (nbCases-1))
    pthread_cond_signal(&casePleinPour[leMessage.type]);
...
void retirer (TypeMessage *unMessage, int rangConso) {
  pthread_mutex_lock(&mutex);
  while((nbLibres == nbCases) || (resCritiques.buffer[resCritiques.iDepot].type != unMessage->type))
    pthread_cond_wait(&casePleinPour[unMessage->type], &mutex);
  retrait(unMessage);
  nbLibres++;
  printf("\t\tConso %d : Message a ete lu = [T%d] %s (de %d)\n",
         rangConso, unMessage->type, unMessage->info, unMessage->rangProd);
  if(nbLibres < nbCases)
    pthread_cond_signal(&casePleinPour[resCritiques.buffer[resCritiques.iDepot].type]);
  pthread_mutex_unlock(&mutex);
}
...
int main(int argc, char *argv[]) {
  ...
  /*-Q3-*/
  pthread_cond_init(&casePleinPour[0], &consommateurFalse);
  pthread_cond_init(&casePleinPour[1], &consommateurTrue);
