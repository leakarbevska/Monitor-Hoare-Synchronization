/*
 * Producteur-consommateur, producteurs déposent de manière alternée 
 *
...
bool prochainType = false;

pthread_mutex_t mutex = PTHREAD_MUTEX_INITIALIZER; //mutex pour func de moniteur
pthread_cond_t casePlein = PTHREAD_COND_INITIALIZER;

pthread_cond_t producteurTrue  = PTHREAD_COND_INITIALIZER;
pthread_cond_t producteurFalse = PTHREAD_COND_INITIALIZER;
...
void deposer (TypeMessage leMessage, int rangProd) {
  ...
  while((nbLibres == 0) || (prochainType != leMessage.type))
...
void retirer (TypeMessage *unMessage, int rangConso) {
  pthread_mutex_lock(&mutex);
  while(nbLibres == nbCases)
    pthread_cond_wait(&casePlein, &mutex);
  retrait(unMessage);
  nbLibres++;
...
int main(int argc, char *argv[]) {
  ...
  /*-Q3-*/
  pthread_cond_init(&caseLibrePour[0], &producteurFalse);
  pthread_cond_init(&caseLibrePour[1], &producteurTrue);
